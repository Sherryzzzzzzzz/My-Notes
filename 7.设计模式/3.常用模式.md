...
 1// 错误：UI直接访问Player  2healthBar.value = GameObject.Find("Player").GetComponent<Player>().Health;  3// 正确：通过事件通知  4public class Player : MonoBehaviour {  5    public event Action<int> OnHealthChanged;  6}  7public class HealthUI : MonoBehaviour {  8    void OnHealthChanged(int health) { /* 更新UI */ }  9}js
