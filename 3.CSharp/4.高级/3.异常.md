...
#### 1. `throw` vs `throw ex`（保留还是重置堆栈跟踪？）
...
{
    // ❌ throw ex：重置堆栈跟踪，原始抛出位置丢失！
    throw ex;

    // ✅ throw：保留原始堆栈跟踪，能追溯到根因
    throw;
}
```

> **结论**：除非你有意隐藏原始异常来源，否则始终用 `throw;`。

#### 2. `InnerException` —— 异常链
...
{
    // ✅ 包装为更有意义的异常，保留原始异常作为 InnerException
    throw new GameDataLoadException("游戏数据加载失败", ex);
}

// 顶层捕获时可以追踪整个异常链
catch (GameDataLoadException ex)
{
...
> **`when` 的优势**：① 比在 catch 块内用 if 判断更清晰 ② 过滤失败不会让堆栈展开（调试时不会中断）

#### 4. `AggregateException` —— 处理并行异常

```csharp
// 多个 Task 的异常会聚合到 AggregateException
try
{
    var tasks = new[] { Task1(), Task2(), Task3() };
    await Task.WhenAll(tasks);
}
catch (Exception ex)  // 只会捕获第一个异常！
{
    Console.WriteLine(ex.Message);
}

// 正确方式：捕获 AggregateException 并展平
try
{
...
#### 5. `ExceptionDispatchInfo` —— 跨上下文重新抛出

```csharp
ExceptionDispatchInfo _captured;

void Capture(Exception ex)
{
    _captured = ExceptionDispatchInfo.Capture(ex);  // 捕获
}

void Rethrow()
{
    _captured?.Throw();  // 保留原始堆栈重新抛出
}
```

> **异常处理性能真相**：try 块本身**几乎零开销**。昂贵的是抛出异常（堆栈展开 + 创建 Exception 对象）。不要为了避免 try-catch 而写脆弱代码。
