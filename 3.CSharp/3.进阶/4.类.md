...
> **⚠️ 上面的代码使用了 Java 风格的 `setLength()`/`getLength()` 方法。在 C# 中，推荐使用**属性（Property）**来替代，更加简洁和符合 C# 习惯。

---

## C# 属性（Property）—— 替代 Java 风格的 getter/setter

C# 属性是对私有字段的封装，语法更简洁，且支持只读/只写/计算属性：
...
public class Box
{
    ...
    public double Weight
    {
        ...
        set
        {
            if (value < 0) throw new ArgumentException("重量不能为负");
            _weight = value;
        }
    }
}

// 使用 —— 比 setLength()/getLength() 更自然
var box = new Box { Length = 6.0, Breadth = 7.0, Height = 5.0 };  // 对象初始化器
Console.WriteLine($"体积: {box.Volume}");  // 输出: 体积: 210
```

> **属性 vs Java 风格方法的对比**：
> ```csharp
> // ❌ Java 风格（不推荐在 C# 中使用）
> box.setLength(10); var len = box.getLength();
>
> // ✅ C# 属性（推荐）
> box.Length = 10; var len = box.Length;
> ```
...
> - `sealed class`：不能被继承
> - `static class`：只能包含静态成员，不能实例化
> - `partial class`：一个类可以分散在多个文件中
