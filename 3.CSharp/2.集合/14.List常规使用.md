...
// 4. 批量设置值
// ⚠️ ForEach 是 Action<T>，对值类型 List（如 List<int>）不会修改元素！
// 因为 lambda 参数 x 是值的副本，x * 2 的结果被丢弃
list1.ForEach(x => Console.WriteLine(x));  // 适合输出/日志等副作用
// 要批量修改值类型 List，请用 for 循环
for (int i = 0; i < list1.Count; i++) list1[i] *= 2;
```
