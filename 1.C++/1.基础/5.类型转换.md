...
> 💡 **注释：为什么需要dynamic_cast？**
>
> 假设你有一个`Animal*`指针，实际指向的是`Dog`对象。你想调用`Dog`特有的方法（比如`bark()`），但编译器只知道这是一个`Animal*`。
>
> ```cpp
...
> ```
>
> 你需要把它转换成`Dog*`才能调用。但如果这个animal实际是`Cat`呢？直接强转就会崩溃。
>
> `dynamic_cast`就是解决这个问题：**安全地判断"这个指针实际指向什么类型"。**
...
>
> **vtable（虚函数表）**：
> - 当类有虚函数时，编译器会为这个类创建一个"函数地址表"
> - 每个对象会存储一个指针（vptr）指向这个表
> - 通过vptr找到vtable，就能实现多态调用
>
> **RTTI（运行时类型信息）**：
> - RTTI是"附加"在vtable上的类型信息
> - 包含类名、继承关系等信息
> - `typeid`和`dynamic_cast`都依赖RTTI
>
> **为什么dynamic_cast要求类有虚函数？**
> - 因为只有有虚函数的类才有vtable
> - 只有有vtable的类才有RTTI
> - 没有RTTI，dynamic_cast就无法在运行时判断类型
