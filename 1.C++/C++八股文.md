...
编译过程分为四个过程：编译（[编译预处理](https://zhida.zhihu.com/search?content_id=237005746&content_type=Article&match_order=1&q=编译预处理&zhida_source=entity)、编译、优化），汇编，链接。

**编译预处理**：处理以 # 开头的指令，产生 .i 文件；

主要的处理操作如下：

- 对全部的#define进行[宏展开](https://zhida.zhihu.com/search?content_id=237005746&content_type=Article&match_order=1&q=宏展开&zhida_source=entity)。
- 处理全部的[条件编译指令](https://zhida.zhihu.com/search?content_id=237005746&content_type=Article&match_order=1&q=条件编译指令&zhida_source=entity)，比方#if、#ifdef、#elif、#else、#endif;
...
- 汇编过程调用[汇编器](https://zhida.zhihu.com/search?content_id=237005746&content_type=Article&match_order=1&q=汇编器&zhida_source=entity)AS来完成，是用于将汇编代码转换成机器可以执行的指令，每一个汇编语句几乎都对应一条机器指令。
- 汇编后的.o文件是纯二进制文件。
...
最基本的链接叫做[静态链接](https://zhida.zhihu.com/search?content_id=237005746&content_type=Article&match_order=1&q=静态链接&zhida_source=entity)，就是将每个模块的源代码文件编译、汇编成目标文件（Linux：.o 文件；Windows：.obj文件），然后将目标文件和库一起链接形成最后的可执行文件（.exe或.out等）。库其实就是一组目标文件的包，就是一些最常用的代码变异成目标文件后打包存放。最常见的库就是运行时库，它是支持程序运行的基本函数的集合。

![img](https://pic2.zhimg.com/v2-a3df06f747649b2d74beb703859c9f67_1440w.jpg)
...
**[动态链接](https://zhida.zhihu.com/search?content_id=237005746&content_type=Article&match_order=1&q=动态链接&zhida_source=entity)**：代码被放到动态链接库或共享对象的某个目标文件中，链接程序只是在最终的可执行程序中记录了共享对象的名字等一些信息。在程序执行时，动态链接库的全部内容会被映射到运行时相应进行的虚拟地址的空间。
...
静态链接是 将各个模块的obj和库链接成**一个完整的可执行程序**；而动态链接是程序在运行的时候寻找动态库的函数符号（重定位），即**DLL不必被包含在最终的exe文件中**；
...
![img](https://picx.zhimg.com/v2-fb0d9bbeccb76343fd30a8da9b8a0f0d_1440w.jpg)
...
![img](https://pic1.zhimg.com/v2-16f5b1d1513e0acf15deae6be7d07aca_1440w.jpg)

**更多面试题讲解可点击下面视频观看**

[![img](https://pic2.zhimg.com/v2-602769acb06a31bfbac1a14d49b13a4d_ipico.jpg)C++后端面试八股文，不仅仅是面试题，刷完就业！_哔哩哔哩_bilibiliwww.bilibili.com/video/BV1kh4y1w7c9/](https://link.zhihu.com/?target=https%3A//www.bilibili.com/video/BV1kh4y1w7c9/)
...
### 1.6. [内存对齐](https://zhida.zhihu.com/search?content_id=237005746&content_type=Article&match_order=1&q=内存对齐&zhida_source=entity)
...
- [智能指针](https://zhida.zhihu.com/search?content_id=237005746&content_type=Article&match_order=1&q=智能指针&zhida_source=entity)：智能指针是 C++ 中已经对内存泄漏封装好了一个工具，可以直接拿来使用，将在下一个问题中对智能指针进行详细的解释。
...
unique_ptr 的使用比较简单，也是用得比较多的智能指针。当我们独占资源的所有权的时候，可以使用 unique_ptr 对资源进行管理——离开 unique_ptr 对象的作用域时，会自动释放资源。这是很基本的 [RAII](https://zhida.zhihu.com/search?content_id=237005746&content_type=Article&match_order=1&q=RAII&zhida_source=entity) 思想。
...
![img](https://pic2.zhimg.com/v2-fb24d92f8cb1884ee7f8c71242b8a3fd_1440w.jpg)
...
![img](https://pic2.zhimg.com/v2-f14e7d5f0d81d1bc3c040d4396f638a7_1440w.jpg)
...
![img](https://pic3.zhimg.com/v2-2b05d2e1f91360adae2a3893876ebb6e_1440w.jpg)
...
![img](https://pic3.zhimg.com/v2-9c44fe928b8343e54089ddaef9185c46_1440w.jpg)

当 shared_ptr 析构并释放共享资源的时候，只要 weak_ptr 对象还存在，控制块就会保留，weak_ptr 可以通过控制块观察到对象是否存活。

![img](https://pic1.zhimg.com/v2-deb3af7c657a1c72a302ec3695b851dc_1440w.jpg)
...
![img](https://pic1.zhimg.com/v2-e66088736cb6023fa56e206e45d921d8_1440w.jpg)

双击我们的代码调用的最后一个函数，这里是CDbQuery::UpdateDatas()，就定位到了申请内存的代码：

![img](https://pic2.zhimg.com/v2-b2a51c8edddbca0756a996b52049d081_1440w.jpg)
...
![img](https://pic1.zhimg.com/v2-f3a612da004d1ad0c4801c19cc2a6c98_1440w.jpg)

![img](https://pic4.zhimg.com/v2-aac573dafa89ef7aef4796d9d1c5bef9_1440w.jpg)
...
**实现方法**：多态是通过**虚函数**实现的，虚函数的地址保存在虚函数表中，虚函数表的地址保存在含有虚函数的类的实例对象的内存空间中。
...
面试中谈 STL 底层时，先说明一个边界：C++ 标准主要规定容器的语义、接口、复杂度和迭代器失效规则，并不规定容器必须使用哪一种具体数据结构。下面提到的“三个指针”“红黑树”“链地址法”等，是 GCC libstdc++ 中常见的实现模型，MSVC 和 libc++ 的内部字段可能不同。
...
#### 6.20.2 vector：连续动态数组

`vector<T>` 的核心目标是提供连续内存上的动态数组。以常见实现为例，内部可以抽象成三个指针：

```cpp
T* begin_;             // 第一个元素
T* end_;               // 已构造元素的尾后位置
T* end_of_storage_;    // 已分配存储空间的尾后位置
```
...
`size` 表示已经构造了多少个 `T` 对象，`capacity` 表示当前内存最多能容纳多少个对象。预留的容量并不代表对应位置已经构造出对象，不能直接读取或调用其析构函数。

当 `push_back` 或 `emplace_back` 后 `size() < capacity()` 时，容器通常只需要在尾部构造一个对象；当容量不足时，基本流程是：

1. 计算新的容量，并分配一块更大的连续内存；
2. 将旧元素移动构造或拷贝构造到新内存；
3. 析构旧内存中的元素并释放旧存储；
4. 更新三个指针，并在尾部构造新元素。

扩容比例是实现细节，标准不保证一定是 2 倍。扩容的目的不是让每次插入都线性，而是通过几何增长使连续 `n` 次尾插的总搬迁成本为 `O(n)`，所以均摊复杂度为 `O(1)`；发生扩容的单次操作可能是 `O(n)`。

扩容时优先移动还是拷贝，和 `T` 是否提供不会抛异常的移动构造有关。若移动构造可能抛异常而拷贝构造可用，实现通常选择拷贝，以维持异常安全保证。

常见接口的含义：

| 操作 | 作用和复杂度 |
| --- | --- |
| `reserve(n)` | 只扩容，不改变 `size`；通常为 `O(n)`，可减少后续扩容 |
| `resize(n)` | 改变元素个数，必要时构造或析构元素 |
| `clear()` | 析构所有元素，但通常不释放容量 |
| `shrink_to_fit()` | 请求收缩容量，但标准不保证一定执行 |
| `operator[]` | 无边界检查，随机访问 `O(1)` |
| `at()` | 有边界检查，越界通常抛出 `std::out_of_range` |
| 中间插入/删除 | 需要移动后续元素，通常为 `O(n)` |

`vector` 的优势是连续内存带来的缓存局部性、随机访问和与 C 接口互操作能力：`data()` 返回首元素地址。代价是中间插入删除慢，扩容可能使地址、引用和迭代器全部失效。

`vector<bool>` 是特化版本，通常按位压缩存储，一个逻辑元素不是一个真正的 `bool` 对象；`operator[]` 返回代理对象而不是 `bool&`。如果需要真正的布尔数组或稳定的引用语义，应谨慎使用它。

#### 6.20.3 string：带小字符串优化的字符容器

`std::string` 具有类似 `vector<char>` 的连续存储性质，`data()` 返回连续字符序列；C++11 后还要求字符串存储连续，C++17 后 `data()` 可以返回可修改字符的指针（不包括尾部的 `\0`）。

主流实现通常使用 SSO（Small String Optimization，小字符串优化）：短字符串直接放在 `string` 对象内部，只有超过内部缓冲区时才申请堆内存。SSO 的具体容量、对象布局和是否采用 SSO 都是实现细节，不能在可移植代码中写死。

字符串的特殊点：

- `size()` 不包含末尾的 `\0`，`c_str()` 保证以 `\0` 结尾；
- 修改操作可能触发重新分配，从而使指针、引用和迭代器失效；
- `reserve`、`resize`、`append` 的含义与 `vector` 类似，但还要考虑 SSO 状态切换；
- 不能把 `c_str()` 返回的指针长期保存后继续修改字符串，修改后该指针可能失效。

#### 6.20.4 array：栈上或对象内部的固定数组

`std::array<T, N>` 本质上是对原生数组 `T[N]` 的轻量封装，通常不包含额外的动态内存和容量字段。它的大小大致为 `N * sizeof(T)`，支持 `begin`、`end`、`size`、`at` 等 STL 接口。

它与 `vector` 的区别是容量和生命周期在编译期确定，不能扩容；与原生数组相比，它能直接使用 STL 算法，并且可以按值复制、赋值。`std::array<T, 0>` 也是合法类型，但不能解引用 `begin()`。

#### 6.20.5 deque：分段连续的双端队列

`deque<T>` 需要在头尾高效插入删除，但又不要求所有元素处于一整块连续内存中。常见实现由三层组成：

```text
deque
  └── 中央控制表（map，保存各个缓冲区的地址）
        ├── buffer 0：一段 T 对象
        ├── buffer 1：一段 T 对象
        └── buffer 2：一段 T 对象
```

这里的 `map` 是“指针数组”，不是 `std::map`。元素被分散放在多个固定大小的缓冲区中，头尾插入通常只需要在当前缓冲区构造对象或增加一个新缓冲区，不需要搬迁全部元素，因此两端插入删除均摊为 `O(1)`。

`deque` 的随机访问仍然是 `O(1)`：实现先根据下标定位缓冲区，再计算元素在缓冲区内的偏移；但它不是连续内存，不能把 `&deque[0]` 当作完整数组传给 C API。由于还要维护控制表和分段边界，其缓存局部性一般不如 `vector`。

`deque` 的迭代器通常比普通指针复杂，里面需要保存当前元素、当前缓冲区边界和缓冲区指针。对 `deque` 进行插入、删除时，迭代器失效规则比 `vector` 更复杂；需要稳定引用时应直接查阅标准对应操作的规则，不能简单套用 `vector` 的结论。

#### 6.20.6 list 和 forward_list：链式结构

`list<T>` 通常是带哨兵节点的双向链表：
...
哨兵节点让头尾插入、删除不必为“是否为空”“是否删除头节点”写大量特殊分支。节点通常单独分配，元素不连续，所以没有随机访问能力；`operator++` 只是沿 `next` 指针移动，`operator+` 不能用于 `list` 迭代器。

已知位置迭代器时，`list` 的插入和删除只需修改相邻指针，复杂度为 `O(1)`，并且不会移动其他节点中的元素。除了被删除节点对应的迭代器、指针和引用，其他元素的迭代器通常保持有效。`splice` 可以在不同链表间转移节点，通常只调整指针，不复制元素。

链表的“插入删除快”有前提：必须已经拿到目标位置的迭代器。寻找位置仍然是 `O(n)`，而且节点分散在堆上，会产生额外指针、分配器和缓存未命中的成本。因此实际工程中即使需要删除，也常优先考虑 `vector` 加索引或延迟删除。

`forward_list<T>` 是单向链表，只保存 `next` 指针，内存开销更小，但只能向前遍历，没有 `size()`，插入删除通常通过 `before` 位置的迭代器完成。

#### 6.20.7 map/set：有序关联容器

`map`、`set`、`multimap` 和 `multiset` 的常见实现是平衡二叉搜索树，主流库一般使用红黑树。节点大致包含：
...
红黑树通过颜色约束保证从根到叶子的最长路径不会超过最短路径的常数倍，使树高保持 `O(log n)`。插入和删除后通过旋转与重新着色恢复约束，旋转只改变局部指针，不需要搬迁整棵树。
...
- 空叶节点视为黑色；
- 红节点不能有红色子节点；
- 从任意节点到其所有后代空叶的每条路径包含相同数量的黑节点。

标准保证有序关联容器的查找、插入和删除为 `O(log n)`，遍历结果按比较器排序。`map` 的元素类型不是 `pair<Key, T>`，而是类似 `pair<const Key, T>`；键被设计为不可修改，否则会破坏树的排序不变量。需要修改键时，应删除旧节点后重新插入，C++17 也可以使用节点句柄 `extract`/`insert`。

`set` 只有键，`map` 保存键值对；`multi` 版本允许等价键重复。比较器要求满足严格弱序，不能简单使用会随机变化或不满足传递性的比较函数。

#### 6.20.8 unordered_map/unordered_set：哈希表

无序关联容器由桶数组、哈希函数和冲突处理结构组成。常见实现是“桶数组 + 链表/节点链”，但标准不强制使用链地址法，具体库也可能采用其他布局。
...
2. 根据桶数量计算桶下标；
3. 在该桶中查找等价键；
4. 若不存在则构造新节点，并将节点挂到冲突结构中；
5. 若负载因子过高，则扩大桶数组并重新分布节点。

平均情况下查找、插入和删除是 `O(1)`，但哈希冲突严重时最坏为 `O(n)`。性能取决于哈希函数、桶数量、键的分布和负载因子，而不是仅仅取决于容器名字。

```cpp
load_factor() = size() / bucket_count()
```

`max_load_factor` 控制允许的最大负载因子；`reserve(n)` 表示希望容纳至少 `n` 个元素，必要时会调整桶数量；`rehash(n)` 直接请求至少 `n` 个桶。rehash 会重新计算元素所属桶，通常会使迭代器失效，但不会因为节点搬迁而使已有元素的引用和指针失效（被删除的元素除外）。

自定义类型作为 key 时，需要同时提供满足要求的哈希函数和相等比较函数。相等的两个键必须产生相同哈希值，否则哈希表将无法正确查找。

#### 6.20.9 priority_queue：堆上的容器适配器

`priority_queue<T, Container, Compare>` 默认使用 `vector<T>` 作为底层容器，并维护堆序。默认比较关系下，`top()` 是最大值。

它的主要操作可以理解为：

- `top()`：返回堆顶，`O(1)`；
- `push`/`emplace`：先放到末尾，再向上调整，`O(log n)`；
- `pop`：移除堆顶，把末尾元素移到根并向下调整，`O(log n)`；
- `make_heap`：由一段数据建堆，复杂度为 `O(n)`，不是逐个 `push` 的 `O(n log n)`。

它不是完整的容器，不提供普通迭代器，因为直接修改任意位置会破坏堆序。如果需要遍历有序结果，通常不断 `pop` 到另一个序列，而不是直接遍历底层容器。

#### 6.20.10 容器适配器 stack 和 queue

适配器通过组合已有容器限制接口，而不是重新实现一套数据结构：

| 适配器 | 默认底层容器 | 语义 |
| --- | --- | --- |
| `stack` | `deque` | 后进先出，LIFO |
| `queue` | `deque` | 先进先出，FIFO |
| `priority_queue` | `vector` | 堆顶优先 |

`stack` 只暴露 `back/push_back/pop_back` 对应的栈操作，`queue` 只暴露 `front/back/push_back/pop_front` 对应的队列操作。只要底层容器满足适配器要求，也可以显式指定 `vector` 或 `list` 等类型。

#### 6.20.11 迭代器为什么能解耦算法和容器

算法只依赖迭代器提供的操作，不依赖容器内部的数据结构。例如 `std::find(first, last, value)` 只要求迭代器能递增并解引用，因此数组指针、`vector` 迭代器和 `list` 迭代器都可以使用它。
...
| Forward | 可多次遍历，支持 `++` |
| Bidirectional | 额外支持 `--` |
| Random access | 支持 `+`、`-`、下标和大小比较 |
| Contiguous | 随机访问且元素连续，C++20 增加 |

`vector` 和 `array` 的迭代器通常属于 contiguous iterator，`deque` 通常属于 random access iterator，`list` 属于 bidirectional iterator，`forward_list` 属于 forward iterator。迭代器类别决定算法能否使用更快的实现，例如 `std::distance` 对随机访问迭代器是 `O(1)`，对链表迭代器是 `O(n)`。

#### 6.20.12 空间配置器、对象构造和析构
...
例如 `vector` 扩容时，先申请一块能够容纳多个 `T` 的原始内存，再使用 placement new、allocator traits 或 `std::construct_at` 在指定地址构造对象；删除元素时先调用对象析构函数，最后才释放原始内存。

所以 `malloc` 只获得字节空间，不会调用 C++ 构造函数；`operator new` 也主要负责分配原始存储，表达式 `new T(...)` 才会进一步构造对象。STL 的 allocator 将这两层职责组合成容器可使用的接口，并允许用户替换内存策略。

本目录的《深入剖析STL内存分配器allocator及其萃取器》进一步介绍了 `allocator`、`rebind` 和 traits；理解容器底层时，应把它与本节的“容器数据结构”分开看：allocator 解决“内存从哪里来”，容器解决“元素如何组织”。

#### 6.20.13 常见容器的迭代器失效

“迭代器失效”本质上是：迭代器内部保存的地址、节点位置或容器版本信息，已经不能再代表原来的元素。典型规律如下，具体操作仍应以标准规则为准：

| 容器 | 常见导致失效的操作 |
| --- | --- |
| `vector` | 扩容会使所有迭代器、指针和引用失效；未扩容时，插入点及其后的迭代器可能失效；删除点及其后的迭代器可能失效 |
| `string` | 重新分配或修改内部存储后，相关迭代器、指针和引用可能失效 |
| `deque` | 插入删除的失效规则复杂，不能按 `vector` 或 `list` 简化判断 |
| `list/forward_list` | 只有被删除节点对应的迭代器、指针和引用失效 |
| `map/set` | 只有被删除节点对应的迭代器、指针和引用失效 |
| `unordered_*` | rehash 会使迭代器失效；删除元素会使该元素失效；其他元素的引用和指针通常保持有效 |

一个典型错误是先保存 `vector` 元素地址，再执行可能扩容的 `push_back`，之后继续使用旧地址。修复方式可以是提前 `reserve`，或者在修改容器后重新获取迭代器、指针和引用。

#### 6.20.14 emplace、完美转发和 erase-remove

`emplace_back(args...)` 在容器存储位置直接调用 `T` 的构造函数，理论上可以减少“先构造临时对象，再移动到容器”的中间步骤；但如果传入的本来就是 `T` 对象，或者构造过程被优化，`push_back` 和 `emplace_back` 的差异可能很小。`emplace` 也不会改变容器的扩容、异常和迭代器失效规则。

对 `vector` 删除满足条件的元素，经典写法是：
...
`remove` 只通过移动元素覆盖待删除值，并不会改变容器的 `size`；真正缩短容器的是 `erase`。C++20 可以使用 `std::erase`/`std::erase_if` 简化表达，但底层仍然要区分“移动元素”和“销毁尾部元素”这两个阶段。

#### 6.20.15 容器选型与复杂度速记

| 需求 | 常用选择 | 原因 |
| --- | --- | --- |
| 连续存储、随机访问、尾部追加 | `vector` | 局部性好，接口简单 |
| 固定大小的连续数组 | `array` | 无动态扩容，开销小 |
| 两端频繁插入删除 | `deque` | 分段存储，两端均摊 `O(1)` |
| 已知位置频繁转移节点 | `list` | 节点稳定，插入删除不搬迁其他元素 |
| 按序遍历、范围查询 | `map/set` | 平衡树，`O(log n)`，天然有序 |
| 只关心快速平均查找 | `unordered_map/set` | 平均 `O(1)`，不保证顺序 |
| 只需要最大值/最小值 | `priority_queue` | 堆顶 `O(1)`，更新 `O(log n)` |

不要仅凭理论复杂度选容器。元素大小、移动成本、缓存局部性、内存分配次数、是否需要稳定地址，往往比一个 `O(1)` 或 `O(log n)` 更影响实际性能。

#### 6.20.16 STL 底层原理面试回答模板

可以按以下顺序回答一个容器的底层原理：

1. 先说数据结构：连续数组、分段数组、链表、平衡树还是哈希表；
2. 再说核心操作如何实现，以及平均/最坏复杂度；
3. 再说内存分配、对象构造和析构是否分离；
4. 最后说迭代器、指针、引用何时失效，以及为什么；
5. 补充标准只保证接口和复杂度，具体字段布局属于实现细节。

例如回答 `vector`：它是连续动态数组，通常由首地址、已构造元素尾指针和容量尾指针管理；容量不足时申请更大的连续空间并移动或拷贝元素，因此尾插均摊 `O(1)`，扩容单次 `O(n)`，中间插入删除 `O(n)`，扩容会使旧迭代器、指针和引用失效；它的优势是随机访问和缓存局部性，代价是扩容和中间修改成本。

### 7 git 分布式版本控制系统

### 7.1 简单说一下大端、小端。

![img](https://pic4.zhimg.com/v2-2bb3a823b5fcb3451e7873f401dac095_1440w.jpg)
...
**小端模式**：低字节存放在低地址，高字节存放在高地址；即**高地址部分权值高，低地址部分权值低**
...
![img](https://pic3.zhimg.com/v2-9ac23fa24fb09eb7b53fcbc76fbfb960_1440w.jpg)
...
### 9.1 栈空间中父类装子类 — 对象切割（Object Slicing）

在栈（Stack）空间中直接按值保存子类对象给父类变量（例如 C++ 中的 `Base b = Derived();`），最核心的问题是会导致**对象切割（Object Slicing）** 和**多态失效**。

**主要问题分析**：

1. **对象切割（Object Slicing）**：栈在编译期就固定了 Base 变量占用的空间大小（`sizeof(Base)`）。当把一个更庞大的 Derived 对象赋值给 Base 时，系统只能将子类中属于父类的成员变量复制过去，子类特有的成员变量会被完全截断丢弃。

2. **多态机制完全失效**：按值赋值会调用父类的拷贝构造函数或赋值重构函数。赋值完成后，栈上的这个对象类型彻底变成了 Base，其虚函数表指针（vptr）指向的是 Base 的虚函数表。通过该变量调用虚函数时，只会执行父类的实现，无法触发子类的多态行为。

3. **无法访问子类特有接口**：由于变量类型就是父类值类型，编译器在编译期就会阻止你调用任何子类独有的成员函数或属性。
...
int main() {
    // ❌ 错误做法：按值存储，触发对象切割
    Base b = Derived();
    b.print(); // 输出 "Base"，多态失效！y 成员被直接丢弃

    // ✅ 正确做法：使用栈上的引用或指针保持多态
    Derived d;
    Base& ref = d;
    ref.print(); // 输出 "Derived"，正确触发多态
}
```

**不同编程语言的差异**：
- **C++**：区分值语义与引用语义。在栈上声明 `Base b = Derived()` 会直接触发对象切割。
- **Java / C#**：栈上存储的统一是对象引用/指针，实际对象存在堆上（例如 `Parent p = new Child();`）。这种情况下不存在对象切割，多态正常生效。

**正确的解法**：若希望在栈空间或栈生命周期内实现父类装子类，应当使用引用或指针：
- 栈上引用：`Base& ref = derivedObj;`（无需动态内存分配，多态生效）
- 栈上原始指针：`Base* ptr = &derivedObj;`（需注意被指向对象的生命周期）
- 智能指针（推荐管理堆内存）：`std::unique_ptr<Base> ptr = std::make_unique<Derived>();`（变量本身在栈上，指向堆上的子类完整对象）

### 9.2 多态基类的虚析构函数（virtual destructor）

当通过父类指针释放一个指向子类对象的堆内存时，如果父类的析构函数没有声明为 virtual，程序将触发**未定义行为（Undefined Behavior）**。最直接的后果是：子类的析构函数根本不会被执行，从而引发内存与资源泄漏。

**不声明虚析构函数的风险**：
- **子类资源泄漏**：如果子类在构造函数中申请了堆内存（如 new）、打开了文件、获取了互斥锁等，因为 `Derived::~Derived()` 没被调用，这些资源将永久泄漏。
- **非预期未定义行为**：C++ 标准明确规定，利用非虚析构函数的基类指针删除派生类对象属于未定义行为，某些编译器实现可能会直接导致程序崩溃或内存损坏。

**虚析构函数的工作机制**：
- **没有 virtual 时（静态绑定）**：编译器仅根据指针类型（`Base*`）决定调用的析构函数。执行 `delete ptr;` 只会直接调用 `Base::~Base()`。
- **声明 virtual 后（动态绑定）**：析构函数被加入虚函数表（vtable）。执行 `delete ptr;` 时，编译器会通过 vtable 查找对象的实际类型，首先调用子类的析构函数 `Derived::~Derived()`，随后自动按继承链向上调用父类的析构函数 `Base::~Base()`。
...
int main() {
    ...
    delete ptr;
    // 输出仅有: "Base destroyed"
    // 风险：Derived 的析构函数未运行，data 数组造成内存泄漏！
}
```

如果在父类中改为 `virtual ~Base() = default;`，输出顺序将变为：
1. `Derived destroyed`（清理子类资源）
2. `Base destroyed`（清理父类资源）

**设计最佳实践**：
- **多态基类**：只要一个类拥有任何虚函数，或者被设计为多态基类，就必须将其析构函数声明为 virtual。
- **不可继承类 / 值类型**：如果类不打算被继承（或声明了 final），不要声明虚析构函数。因为加入虚函数会导致对象额外携带一个虚函数表指针（vptr，占 8 字节），破坏其内联性能和紧凑的内存布局。
- **纯虚析构函数**：可以将析构函数声明为纯虚函数 `virtual ~Base() = 0;`，但同时必须提供函数体定义。这样既能让类成为抽象类（不能实例化），又能保证析构时正确调用析构链。

### 9.3 C++ 类可以在栈上 vs C# 引用类型必须在堆上

**根本原因在于语言的设计哲学以及内存管理方式（手动 vs 垃圾回收）的不同。**

**C++：默认采用"值语义"（Value Semantics）**
- C++ 的理念是"为你不需要的东西零付出"。类型的定义与它的存储位置无关，任何类型（无论是基本类型还是自定义 class）默认都可以直接分配在栈上。
- 编译期即可精准计算栈上对象的大小（`sizeof(Class)`）。当函数执行完毕离开作用域时，栈指针直接回退，自动调用析构函数清理资源，极其高效。

**C#：基于 CLR 托管与 GC（垃圾回收）机制**
- C# 将类型严格划分为**值类型（struct）**和**引用类型（class）**。
- class 实例化出的对象需要由 CLR 的 GC 进行生命周期追踪。为了实现 GC 的追踪、跨作用域传递以及多态，C# 堆上的每个对象头部都有额外的元数据（如 Sync Block Index 和 TypeHandle / MethodTable 指针）。
- 栈帧会随着函数返回被销毁，但引用类型对象的生命周期可能长于当前栈帧，因此必须存放在 GC 堆中，而栈上仅保留指向该堆内存的引用指针。
- 注：C# 的 struct 是值类型，可以直接存放在栈上。

**对比总结**：

| 维度 | C++ | C# |
|------|-----|-----|
| class 存储位置 | 栈或堆（任意） | 只能在托管堆 |
| struct 存储位置 | 栈或堆（任意） | 栈（局部变量时） |
| 内存管理 | 手动（new/delete）或 RAII | GC 自动回收 |
| 对象生命周期 | 由作用域/程序员决定 | 由 GC 可达性决定 |
| 多态实现 | 指针/引用 + virtual | 引用（天然指针语义） |

### 9.4 C# 的指针类似机制

C# 提供了从"绝对安全"到"底层原生"的多个层级：

| 机制 | 说明 |
|------|------|
| **ref / out / in 关键字** | 托管引用/安全指针。允许在不复制值的情况下传递变量的内存地址，相当于安全的受控指针 |
| **Span\<T\> 与 ref struct** | 类似于 C++17 的 `std::span`，允许安全地以指针效率直接操作连续内存（无论是栈内存、堆内存还是非托管内存），无需发生内存拷贝 |
| **unsafe 代码块（裸指针）** | C# 允许声明 unsafe 上下文，直接使用类似 C/C++ 的原生指针语法（`*`、`&`、`->`）。在操作托管堆对象时，需搭配 `fixed` 关键字"钉住"内存，防止 GC 在垃圾回收时移动该对象 |
| **IntPtr / nint（原生整数指针）** | 用于与 C/C++ 底层 API 交互（P/Invoke 平台调用）时传递内存地址 |

### 9.5 C++ 的引用是不是用指针实现的？

**从语法概念上说不是，但在底层汇编和编译器实现上，几乎总是指针。**

- **标准定义**：在 C++ 语言标准中，引用被定义为已有对象的"别名"（Alias），标准并未规定其具体底层实现方式。
- **编译器实现**：在绝大多数现代编译器（GCC、Clang、MSVC）中，引用的底层汇编指令和 `T* const`（指针常量）完全一致，同样占用一个指针大小的内存（32位占 4 字节，64位占 8 字节）。
- **编译器优化（例外）**：如果引用仅仅是局部变量的别名，或者被编译器进行了函数内联（Inlining）优化，编译器会在编译期直接将其替换为原始变量，此时完全不会产生任何指针指令，也不占用额外内存。

**总结**：引用在语义上是别名，在实现上是指针。这种设计让引用既有指针的高效（零拷贝传递），又有值语义的简洁（无需解引用操作符）。
